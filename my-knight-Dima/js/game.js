...
attackImage.src = './assets/images/Attack.png';
...
const attackCooldown = 500; // время кд короче если нужно меняй Димас
...
if (!player.isJumping) player.action = 'run';
  } else if (keys.left) {
       player.x -= player.speed;
    player.direction = 'left';
     if (!player.isJumping) player.action = 'run';
   } else {
      if (!player.isJumping && !keys.attack) player.action = 'idle';
   }
...
function drawPlayer() {
    ...
    else if (player.action === 'attack') sprite = attackImage;
    else if (player.action === 'jump') sprite = jumpImage;
    else sprite = idleImage;
...
function drawGround() {
    for (let i = -1; i < canvas.width / 64 + 2; i++) {
        ctx.drawImage(dirtImage, i * 64 - camera.x % 64, groundY, 64, 64);
    }
}
